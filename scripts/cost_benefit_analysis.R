...
pgm.avoiddeforest.cost.total <- pgm.costs.total[["PGM_2010_real_base_opportunity"]]
...
pgm.avoidboth.cost.total <- pgm.costs.total[["PGM_2010_real_base_firecontrol"]] + pgm.costs.total[["PGM_2010_real_base_opportunity"]] + pgm.costs.total[["PGM_2010_real_base_haverst"]]
...
pgm.restor_n_avoid_deforest.cost.total <- pgm.costs.total[["PGM_2010_real_base_restoration"]] + pgm.costs.total[["PGM_2010_real_base_opportunity"]]
...
pgm.restor_n_avoid_both.cost.total <- pgm.costs.total[["PGM_2010_real_base_firecontrol"]] + pgm.costs.total[["PGM_2010_real_base_restoration"]] + pgm.costs.total[["PGM_2010_real_base_opportunity"]] + pgm.costs.total[["PGM_2010_real_base_haverst"]]
...
stm.avoiddeforest.cost.total <- stm.costs.total[["STM_2010_real_base_opportunity"]]
...
stm.avoidboth.cost.total <- stm.costs.total[["STM_2010_real_base_firecontrol"]] + stm.costs.total[["STM_2010_real_base_opportunity"]] + stm.costs.total[["STM_2010_real_base_haverst"]]
...
stm.restor_n_avoid_deforest.cost.total <- stm.costs.total[["STM_2010_real_base_restoration"]] + stm.costs.total[["STM_2010_real_base_opportunity"]]
...
stm.restor_n_avoid_both.cost.total <- stm.costs.total[["STM_2010_real_base_firecontrol"]] + stm.costs.total[["STM_2010_real_base_restoration"]] + stm.costs.total[["STM_2010_real_base_opportunity"]] + stm.costs.total[["STM_2010_real_base_haverst"]]
...
# comparing benefits considering all possibilities =============================
#converting to dataframe
...
rm(stm.biodiversity.benefit.total); rm(stm.biodiversity.benefit)
gc()
#
...
biodiv.summation <- biodiversity.benefit.birds %>%
  ...
  mutate(sum.benefit = net.change - min(net.change),
         area = factor(case_when(str_detect(Scenario, "(PF only)") ~ 1,
                                 .default = 0),
                       levels = c(0,1),
                       labels = c("All", "Primary forest")),
         Scenario = factor(Scenario,
                           labels = c("Business as usual",
                                      "Avoid deforestation", 
                                      "Avoid degradation",
...
biodiversity.benefit.birds %>% 
  ...
  group_by(Region) %>% sample_n(100000) %>% {
    ggplot(.) +
      ...
      geom_violin(aes(x = Scenario, y = BBenefit_ha.real), alpha = 0, scale = "width") +
      geom_hline(yintercept = 0, linetype = "dashed", linewidth = 1.2, colour = "gray33") +
      scale_x_discrete(limits = rev(c("Observed changes",
                                      "Avoided deforestation",
                                      "Avoided degradation",
                                      "Restoration only",
                                      "Avoided deforestation and degradation",
...
rm(pgm.carbon.benefit.total); rm(pgm.carbon.benefit)
gc()
#
...
rm(stm.carbon.benefit.total); rm(stm.carbon.benefit)
gc()
#
...
carbon.benefit <- carbon.benefit %>% 
  mutate(Region = factor(Region, levels = c("PGM", "STM")),
         CBenefit_ha = CBenefit*0.08919563)

rm(pgm.carbon.benefit.df); rm(stm.carbon.benefit.df) 
gc()


carb.real <- carbon.benefit %>% filter(Scenario == "Business as usual") %>% droplevels() %>% 
  dplyr::select(Region, Cell, CBenefit_ha) %>% dplyr::rename(CBenefit_ha.real = CBenefit_ha)

carbon.benefit <- carbon.benefit %>% left_join(carb.real)
...
carb.summation <- carbon.benefit %>%
  group_by(Region, Scenario) %>% 
  summarise(net.change = sum(CBenefit)) %>% 
  mutate(sum.benefit = net.change - min(net.change),
         area = factor(case_when(str_detect(Scenario, "(PF only)") ~ 1,
                                 .default = 0),
                       levels = c(0,1),
  ...
  group_by(Region) %>% sample_n(100000) %>% {
    ggplot(.) +
      ...
      scale_x_discrete(limits = rev(c("Observed changes",
                                      ...
                                      "Restoration and avoided deforestation",
                                      "Restoration and avoided deforestation and degradation"))) + 
      scale_y_continuous("Carbon difference", 
                         limits = c(-18,18)) +#,
      #sec.axis = sec_axis(~ ., breaks = c(-1,-0.5,0,0.5,1), labels = ly.lbl,
      #                    name = "")) +
      scale_color_manual(values = c("#294B2985", "#76453B85", #"#50623A85","#B19470",
